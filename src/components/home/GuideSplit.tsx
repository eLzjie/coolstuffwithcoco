...
import type { HomeArm } from "@/components/home/Hero";
 ...
 *
 * ---------------------------------------------------------------------------
 * THE ARM DECIDES BOTH THE ORDER AND THE DESTINATION
 * ---------------------------------------------------------------------------
 * `/b` sends visitors to `/decode/b` and `/vetbill/b`, and leads with the Vet
 * Bill rather than Decode. That reordering is the reason the two halves are
 * separate components below instead of one long block: swapping two sibling
 * JSX blocks by hand is how one of them quietly keeps the wrong entrance
 * animation.
 *
 * Which is the other thing to know — `reveal-left` / `reveal-right` follow
 * POSITION, not identity. They are passed in, so whichever half renders first
 * parts to the left. Baking the class into each half would have both of them
 * sliding the same way once the order flips.
 */

type Half = {
  /** Entrance direction, by position in the grid — see the note above. */
  reveal: "reveal-left" | "reveal-right";
  /** Arm-correct destination. */
  href: string;
};

/*
  `page_path` is passed explicitly rather than left to gtag's implicit page
  location. `/` and `/b` fire the same `view_item` with the same lead magnet,
  so without it the two home arms are indistinguishable in GA4 — and the arm
  is the entire question this page exists to answer.
*/
const clickProps = (magnet: "decode" | "vetbill", title: string) => ({
  lead_magnet: magnet,
  content_name: title,
  from: "home_split",
...
function DecodeHalf({ reveal, href }: Half) {
  return (
    <div className={`${reveal} relative overflow-hidden bg-bubblegum px-[clamp(1.5rem,5vw,4rem)] py-16`}>
      ...
      */}
      <div className="mt-8 flex flex-col items-start gap-5 sm:flex-row sm:items-end sm:gap-6">
        ...
        <Link
          ...
          onClick={() => track("view_content", clickProps("decode", DECODE.title))}
        >
          Read her signals
        </Link>
      </div>
...
function VetbillHalf({ reveal, href }: Half) {
  return (
    <div className={`${reveal} relative overflow-hidden bg-sky px-[clamp(1.5rem,5vw,4rem)] py-16`}>
      ...
      */}
      <div className="mt-8 flex flex-col items-start gap-5 sm:flex-row sm:items-end sm:gap-6">
        ...
        <Link
          href={href}
          className="btn-coral whitespace-nowrap"
          onClick={() => track("view_content", clickProps("vetbill", VETBILL.title))}
        >
          Be ready for it
        </Link>
      </div>
...
export function GuideSplit({ arm = "a" }: { arm?: HomeArm }) {
  const b = arm === "b";

  const decode = <DecodeHalf key="decode" reveal={b ? "reveal-right" : "reveal-left"} href={b ? "/decode/b" : "/decode"} />;
  const vetbill = <VetbillHalf key="vetbill" reveal={b ? "reveal-left" : "reveal-right"} href={b ? "/vetbill/b" : "/vetbill"} />;
  ...
  return (
    <section id="guides" className="scroll-mt-8" aria-labelledby="guides-heading">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        {b ? [vetbill, decode] : [decode, vetbill]}
      </div>
    </section>
  );
}
